<template>
    <div class="parking-container">
        <template v-if="loading">
            <div class="spinner-border" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            <div>Proszę czekać ...</div>
        </template>
        <template v-else-if="passes == null">
            <div class="alert alert-warning">Nie udało się pobrać stanu parkingu.</div>
        </template>
        <template v-else>
            <div class="card">
                <div class="card-body">
                    <div class="card-title">Parking dla niepełnosprawnych (SRP)</div>
                    <UsageSummary :used="usedCount" :total="passes.length" />
                    <div class="pass-grid">
                        <PassBadge v-for="p in passes" :key="p.id" :pass="p" />
                    </div>
                </div>
            </div>

            <!-- Legenda stanów -->
            <div class="card">
                <div class="card-body">
                    <div class="card-title">LEGENDA</div>
                    <div class="legend-row">
                        <PassBadge :pass="{ zbor: 100, nr: 1, used: false }" /> - nr zboru/nr identyfikatora — wolny
                    </div>
                    <div class="legend-row">
                        <PassBadge :pass="{ zbor: 100, nr: 2, used: true }" /> - nr zboru/nr identyfikatora — użyty (pojazd na parkingu)
                    </div>
                </div>
            </div>

            <small>Stan z: {{ timestamp }}</small>
        </template>
    </div>
</template>

<script setup>
import { ref, computed, onMounted, onBeforeUnmount } from 'vue';
import UsageSummary from '@/components/parking/UsageSummary.vue';
import PassBadge from '@/components/parking/PassBadge.vue';
import { useOdprawaSocket } from '@/composables/useOdprawaSocket.js';

const loading = ref(true)
const passes = ref(null)
const timer = ref(null)
const timestamp = ref(undefined)

let socket = null

onMounted(() => {
    // Kanał główny: WebSocket; rekoncyliacja pełnym fetchem po (re)połączeniu
    socket = useOdprawaSocket(['parking:srp'], null, loading_usage, apply_ws_parking)

    loading_usage()

    // Siatka bezpieczeństwa: wolny polling (co 60s) gdy WS rozłączony
    timer.value = setInterval(() => {
        if (!socket.isConnected.value)
            loading_usage()
    }, 60000)
})

onBeforeUnmount(() => {
    if (timer.value != null) {
        clearInterval(timer.value)
        timer.value = null
    }
})

const usedCount = computed(() =>
    passes.value ? passes.value.filter(p => p.used).length : 0
)

// Pobieranie pełnego stanu użycia identyfikatorów (init + fallback)
function loading_usage() {
    fetch('/api/monitoring/parking/srp')
    .then(response => {
        if (response.status === 200)
            return response.json()
        else
            console.error("Loading srp parking usage:", response)
    })
    .then(d => {
        if (d) {
            passes.value = d.passes
            timestamp.value = new Date().toLocaleTimeString()
        }
        loading.value = false
    })
    .catch(err => {
        console.error("Loading srp parking usage:", err)
        loading.value = false
    })
}

// Aktualizacja stanu z ramki WebSocket (delta update)
function apply_ws_parking(json) {
    if (json.parking !== 'srp' || passes.value == null)
        return
    const pass = passes.value.find(p => p.id === json.id)
    if (pass) {
        pass.used = json.used
        pass.ts = json.used ? json.ts : undefined
        timestamp.value = new Date().toLocaleTimeString()
    }
}
</script>

<style scoped>
.parking-container {
    padding: 16pt 6pt;
    display: flex;
    flex-direction: column;
    gap: 24pt;
}
.pass-grid {
    margin-top: 9pt;
    display: flex;
    flex-direction: row;
    flex-wrap: wrap;
    gap: 4pt;
}
.legend-row {
    display: flex;
    flex-direction: row;
    align-items: center;
    gap: 5pt;
    margin-bottom: 3pt;
}
</style>
